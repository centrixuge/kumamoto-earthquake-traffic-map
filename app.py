...
from modules import datastore, docomo_od, transtron
...
def main():
    ...
    (tab_mlit, tab_mesh, tab_mesh_cmp, tab_overview, tab_dl,
     tab_probe, tab_docomo_od) = st.tabs(
        [
            "道路規制×交通量",
            "人口推計値×交通量",
            "人口推計値の時点比較",
    ...
    # ------------------------------------------------------------------
    # ドコモODデータタブ
    # 提供の条件（クレジット・データ内容の説明・利用範囲）は
    # modules/docomo_od.py の先頭に直接書いて必ず出す
    # ------------------------------------------------------------------
    with tab_docomo_od:
        docomo_od.render_tab()
