...
# 選択中の観測点の色。地図のリングと時系列の折れ線で共用する。
# 水色とオレンジは補色に近く、2地点の折れ線を見分けやすい。
# 赤（全面通行止め #e60000）は避ける。オレンジは片側交互通行止め
# （#e67e22）と近いが、そちらは大半が解除済みで破線・半透明のため
# 実線で残る本数が少なく、選択色の方を明るくして見分けをつけている。
SELECTION_COLORS = ["#00a3d9", "#ff8c1a"]
        ...
        if post.empty:
            ...
            with col_map:
                ...
                st.caption(
                    ...
                    "選択中の観測点は水色/オレンジの枠で強調表示され、時系列の折れ線も同じ色になります（最大2地点）。"
