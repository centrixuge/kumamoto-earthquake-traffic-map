...
| タブ | 内容 |
| --- | --- |
| 地図・時系列 | 観測点別の異常度（zスコア）と通行規制情報を地図上に表示し、クリックした観測点（最大2地点）の時系列（平常時帯 vs 実測）を並べて表示 |
...
- **異常検知**: 観測点×時刻(hour)ごとに、平常時（地震前2週間の同曜日ペア、2026-07-14/15・07-21/22）の交通量の平均・標準偏差を算出し、地震発生後の実測値とのzスコアを計算。`|zスコア| >= 2` を異常候補としてフラグ。地図の色分けと異常検知一覧はこの5分間値ベースの定義を使っています
- **時系列の3ビュー**: 5分間値は過去1ヶ月しか遡れず平常時が2日分しか取れないため、3ヶ月遡れる1時間値から同曜日8週分の平常時も用意し、時系列図で切り替えられます
  - `5分間値（平常時は直近2日）`: 従来どおり
  - `5分間値（平常時は1時間値8週分÷12）`: 平常時を1時間値から求め、単位を合わせるため1/12。**この帯は「平常時の1時間あたり交通量の日々のばらつき÷12」であり、5分間値そのもののばらつきではないため狭く出ます**（実測σ 9.2 に対し 3.2 程度）
  - `1時間値（参考・平常時も1時間値8週分）`: 実績・平常時とも1時間値どうしの比較で、統計的にはこれが素直
