...
| 道路通行規制情報（**直轄国道**） | 国土交通省 九州地方整備局 [熊本河川国道事務所](https://www.qsr.mlit.go.jp/kumamoto/)の「事務所からのお知らせ」で公表されるPDF | **上のポータルには直轄国道の規制が載らない**ため、PDFから**手作業で転記**して `data/mlit_regulations.json` に持っています。自動取得ではありません（次節参照） |
...
この規制はポータルのデータに存在しないため、PDFから転記して `data/mlit_regulations.json` に持ち、ダッシュボードでは次のように扱っています。
