...
| データダウンロード | アーカイブ済みの交通量（5分値・1時間値）と通行規制（一覧・状態変化履歴）をCSVで取得。各列の意味・単位をまとめた**列定義書**を**Excel（xlsx）**でダウンロードできます（データセットごとにシートを分け、先頭に目次「はじめに」シートを置いています。実データの列と自動照合し、「配布中CSVに存在」列で確認できます。食い違いはエラーではなく起こりうるものなので、理由の分かる備考としてシートと画面の両方に載せます） |
