...
| データダウンロード | アーカイブ済みの交通量（5分値・1時間値）と通行規制（一覧・状態変化履歴）をCSVで取得。各列の意味・単位をまとめた**列定義書**を**Excel（xlsx）**でダウンロードできます（データセットごとにシートを分け、先頭に目次「はじめに」シートを置いています。実データの列と自動照合し、食い違いがあるときだけ「配布中CSVに存在」列と理由の分かる備考が追加されます。食い違いはエラーではなく一時的に起こりうるものです） |
...
- 実行のたびに、アーカイブ済みの**コマ（5分間値なら1コマ=5分）と期待されるコマの並びを突き合わせ、まだ持っていないコマだけ**をAPIに問い合わせます。1コマ=1リクエストなので、範囲でまとめて取り直すより無駄がなく、途中に空いた穴も次の実行で埋まります
- JARTIC側がそもそも配信していないコマ（リクエストしても0件が返る）が時々あります。この場合はデータが存在しないので埋まりません。時系列図ではその区間だけ線が途切れます
