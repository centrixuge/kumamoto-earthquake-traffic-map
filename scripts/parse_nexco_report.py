...
SECTION_EMERGENCY = re.compile(r"[23]\.\s*緊急車両通行可能区間\s*（([^）]*)）")
SECTION_CLOSURE = re.compile(r"[34]\.\s*通行止め状況\s*（([^）]*)）")
# 「1. 緊急車両通行可能となる区間／見込み区間」。これから開く区間の告知で、
# 2.の一覧（○○時点）にはまだ入っていない。反映するかは人が決める。
SECTION_BECOMING = re.compile(r"1\.\s*緊急車両通行可能となる(?:見込み)?区間")
BECOMING_TIME = re.compile(
    r"(\d{1,2})\s*月\s*(\d{1,2})\s*日.{0,12}?(\d{1,2})\s*時\s*(\d{2})\s*分"
)
NEXT_SECTION = re.compile(r"\n\s*\d\.\s*[^\n]")
AS_OF = re.compile(r"(\d{1,2})\s*/\s*(\d{1,2})\s*([0-9０-９]{1,2})\s*[:：]\s*([0-9０-９]{2})")
