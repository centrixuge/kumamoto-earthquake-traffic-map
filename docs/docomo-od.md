...
「集計軸 × 期間（震災前・震災後）」の6ファイル（CP932）。**震災後だけ氷川町断面の方向別**です。**このリポジトリは公開なので、期間・区分・件数といったデータの中身はここには書きません。** 画面に出す説明文も同じ理由で `docomo_od_meta.json` の `display` に入れてあり、アプリが実行時に読みます（提供の条件3点だけはアプリのコードに直接書いてあり、置き場が読めなくても必ず出ます）。
