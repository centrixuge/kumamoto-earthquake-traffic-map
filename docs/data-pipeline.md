...
- GitHub Actions（後述）で定期実行することで、収集の終端（`TARGET_END_CAP` = 2026-09-07 03:00）まで自動的にアーカイブが育っていきます
...
収集の終端（`TARGET_END_CAP` = 2026-09-07 03:00）を過ぎるとフェッチ対象がなくなり実質no-opになりますが、ワークフロー自体は動き続けます。不要になったら GitHub の Actions 設定からスケジュールを無効化してください。**期間をさらに伸ばすなら、5分間値が遡れる1ヶ月以内に上限を動かしてください**（遅れるとその間の5分間値は取れなくなります）。
